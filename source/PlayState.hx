...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (OpenFlAssets.exists('assets/scripts/funnyhealthbar.lua')) {
		  doPush = true;
		}
		
		if(doPush)
		luaArray.push(new FunkinLua(Asset2File.getPath('assets/scripts/funnyhealthbar.lua')));
		#end
		
		#if LUA_ALLOWED
		var doPush:Bool = false; 
		if (OpenFlAssets.exists('assets/scripts/holdFix.lua')) {
		  doPush = true;
		}
		
		if(doPush)
		luaArray.push(new FunkinLua(Asset2File.getPath('assets/scripts/holdFix.lua'))); // It doesn't have to be "script1", "script2", you can put whatever name you want. 
		#end
		...
		#if LUA_ALLOWED
		var doPush:Bool = false;
		if (OpenFlAssets.exists('assets/data/' + Paths.formatToSongPath(SONG.song) + '/dodge.lua')) {
		  doPush = true;
		}
		
		if(doPush)
		luaArray.push(new FunkinLua(Asset2File.getPath('assets/data/' + Paths.formatToSongPath(SONG.song) + '/dodge.lua')));
		#end
